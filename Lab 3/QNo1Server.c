...
//check if id is used... if it is not used returns true else false;
bool checkID(char id, char str[] ){
        bool retVal = false;
        for(int i=0; i < 10; i++){
                if(!(str[i] >= '0' && str[i] <='9')){
                        str[i] = id;
...
int main(void)
{
        ...
        while(clientsIDs[9] == '\0'){
                ...
                char clientID = client_message[strlen(client_message)-1];
                //Send the message back to client
                if (clientID >= '0' && clientID <='9' && checkID (clientID,clientsIDs))
                        sprintf(server_message, "Hello I am server. Your received ID is %c", clientID);
                else
                        sprintf(server_message, "Hello I am server. Your received ID %c is already used", clientID);
